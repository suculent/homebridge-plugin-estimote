...
module.exports = (homebridge) => {
  ...
  homebridge.registerAccessory('homebridge-estimote', 'Estimote', EstimotePlugin);
};
...
class EstimotePlugin {
  ...
  constructor(log, config)
  {
    ...
    this.broker_address = config.mqtt_broker || broker_address
    ...
    {
      that.log("MQTT connected, subscribing to: " + that.mqtt_channel + "/set")
      mqttClient.subscribe(this.mqtt_channel + "/set")

      that.log("Registering BLE Transport Listener")

      var noble = require('noble')

      noble.on('stateChange', function(state) {        
        if (state == 'poweredOn') {
          that.log('Noble: Estimote service  state has changed to ON');
          ...
          noble.startScanning(serviceUUIDs, allowDuplicates, function(error) {
            if (error) {
              that.log('Noble: Error starting scanning.', error);
            } else {
              that.log('Noble: Scanning for services...');
            }
          });
        } else {
          that.log('Noble: Estimote service state has changed to OFF');
        }
      });
  ...
  getServices()
  {
    var informationService = new Service.AccessoryInformation()
        ...
        .setCharacteristic(Characteristic.Model, "Estimote Telemetry")
    ...
    var lightService = new Service.LightSensor(this.name, "light")
        lightService
    ...
    var contacts = this.contacts      
        contacts.push(batteryService)
        contacts.push(lightService)
        contacts.push(temperatureService)
        contacts.push(informationService)
  ...
  /* Estimote Telemetry Packet Parser */
  parseEstimoteTelemetryPacket(data) { // data is a 0-indexed byte array/buffer
    ...
    if (subFrameType == ESTIMOTE_TELEMETRY_SUBFRAME_A) {
      ...
      function parseMotionStateDuration(byte) {
      ...
      */
      ...
      return {
        ...
        "isMoving" : isMoving,
        "motionStateDuration" : motionStateDuration,
        "pressure" : pressure,
        "gpio" : gpio,
        "errors" : errors
      };
